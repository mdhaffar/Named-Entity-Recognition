...
class ASR(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        features_matrix = batch.feats
        out = features_matrix[0]
        #torch.nn.utils.rnn.pad_sequence(features_matrix, batch_first=True)
        self.y = self.modules.enc2(out)
        self.z = self.modules.enc3(self.y)
        logits = self.modules.ctc_lin(self.z)
        '''print(logits)
        print('len logits')
        print(logits.size())'''
...
#    valid_data = valid_data.filtered_sorted(sort_key="duration")
    ...
    @sb.utils.data_pipeline.takes("id", "start_seg", "end_seg")
    @sb.utils.data_pipeline.provides("sig")
    def audio_pipeline(id, start_seg, end_seg):
#        info = torchaudio.info(wav)
        
        start = int(float(start_seg) * hparams["sample_rate"])
        stop = int(float(end_seg) * hparams["sample_rate"])     
        speech_segment = {"file" : id+'.wav', "start" : start, "stop" : stop}
    ...
    def np_pipeline(wav):
        feats = np.load(wav,allow_pickle=True)
        if('smdhaffar' not in wav):
...
#        return resampled
    ...
    sb.dataio.dataset.add_dynamic_item(datasets, np_pipeline)
    ...
    sb.dataio.dataset.set_output_keys(
        datasets, ["id", "sig", "feats" ,"char_encoded"],
    )
